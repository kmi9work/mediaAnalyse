...
window.trackOn = (id) -> 
	$.ajax(url: "/queries/" + id + "/start_work").done (html) ->
  		$("#track_button" + id).html("<button type=\"button\" class=\"btn btn-success\" onclick=\"trackOff(" + id + ")\">Остановить</button>")

window.trackOff = (id) -> 
	$.ajax(url: "/queries/" + id + "/stop_work").done (html) ->
  		$("#track_button" + id).html("<button type=\"button\" class=\"btn btn-primary\" onclick=\"trackOn(" + id + ")\">Отслеживать</button>")
